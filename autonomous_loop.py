...
async def run_autonomous_loop(
    ...
    for iteration in range(max_iterations):
        ...
        # Gates run only when needed based on task characteristics
        print(f"🛡️  Running meta-agent gates...")

        # GATE 1: Governance Agent (ALWAYS runs - risk assessment)
        ...
        print(f"   Governance: {governance_result.decision} (risk: {governance_result.risk_assessment.risk_level})")

        if governance_result.decision == "BLOCKED":
            queue.mark_blocked(task.id, f"Governance: {governance_result.risk_assessment.reason}")
            queue.save(queue_path)
            update_progress_file(actual_project_dir, task, "blocked", f"Governance blocked: {governance_result.risk_assessment.reason}")
            continue
        elif governance_result.decision == "REQUIRES_APPROVAL":
            if non_interactive:
                print(f"\n⚙️  Non-interactive mode: Auto-approving {governance_result.risk_assessment.risk_level} risk task")
            else:
                print(f"\n{'='*80}")
                print(f"⚠️  GOVERNANCE APPROVAL REQUIRED")
                print(f"{'='*80}\n")
                ...
                for rec in governance_result.recommendations:
                    print(f"  - {rec}")
                print()

                response = input("Approve this task? [y/N]: ").strip().lower()
                if response != 'y':
                    queue.mark_blocked(task.id, f"Governance: Rejected by user")
                    queue.save(queue_path)
                    update_progress_file(actual_project_dir, task, "blocked", "Governance: Rejected by user")
                    continue

        # GATE 2: Product Manager (CONDITIONAL - only if feature OR affects users)
        should_pm_validate = (
            (hasattr(task, 'type') and task.type == "feature") or
        ...
        if should_pm_validate:
            ...
            print(f"   PM: {pm_result.decision} ({pm_result.evidence_count} evidence items)")

            if pm_result.decision == "BLOCKED":
                queue.mark_blocked(task.id, f"PM: {pm_result.reason}")
                queue.save(queue_path)
                update_progress_file(actual_project_dir, task, "blocked", f"PM blocked: {pm_result.reason}")
                continue
            elif pm_result.decision == "MODIFIED":
                # PM added outcome metrics template
                if pm_result.modified_description:
                    task.pm_outcome_metrics = "Added by PM"
                    print(f"   PM modified task: {pm_result.reason}")
        else:
            print(f"   PM: Skipped (not a feature, no user impact)")

        # GATE 3: CMO Agent (CONDITIONAL - only if GTM-related)
        ...
        if should_cmo_review:
            ...
            task.cmo_reviewed = (cmo_result.decision == "APPROVED")

            print(f"   CMO: {cmo_result.decision}")

            if cmo_result.decision == "PROPOSE_ALTERNATIVE":
                # CMO proposes alternative (doesn't block, but warns)
                print(f"   CMO suggests: {cmo_result.proposed_alternative}")
                if not non_interactive:
                    print(f"\n{'='*80}")
                    print(f"⚠️  CMO PROPOSES ALTERNATIVE")
                    print(f"{'='*80}\n")
                    print(f"Task {task.id}:")
                    print(f"Current: {task.description}")
                    print(f"Suggested: {cmo_result.proposed_alternative}\n")

                    response = input("Proceed with original plan? [y/N]: ").strip().lower()
                    if response != 'y':
                        queue.mark_blocked(task.id, f"CMO: User chose alternative approach")
                        queue.save(queue_path)
                        update_progress_file(actual_project_dir, task, "blocked", "CMO: User chose alternative")
                        continue
        else:
            print(f"   CMO: Skipped (not GTM-related)")

        print(f"✅ All meta-agent gates passed\n")

        # Save task updates (meta-agent fields populated)
        queue.save(queue_path)
