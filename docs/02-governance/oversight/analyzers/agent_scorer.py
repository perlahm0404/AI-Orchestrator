...
def generate_scorecard(agent_performance: Dict[str, Any]) -> List[AgentScore]:
    """
...
def _calculate_score(agent_type: str, perf: Dict[str, Any]) -> AgentScore:
    """Calculate score for a single agent."""
    total_tasks = perf.get("total_tasks", 0)
    completion_rate = perf.get("completion_rate", 0.0)
    ralph_pass_rate = perf.get("ralph_pass_rate", 0.0)
    avg_iterations = perf.get("avg_iterations", 0.0)
