...
def _detect_unused_agents(agent_performance: Dict[str, AgentPerformance]) -> List[BloatIndicator]:
    ...
    for agent_type, perf in agent_performance.items():
        ...
        # Low utilization threshold: <5 tasks total with high idle sessions
        if total_tasks < 5 and perf.idle_sessions > 20:
            bloat.append(BloatIndicator(
                type="unused_agent",
                component=agent_type,
                usage_count=total_tasks,
                recommendation=f"Sunset '{agent_type}' agent - low utilization ({total_tasks} tasks, {perf.idle_sessions} idle sessions)",
